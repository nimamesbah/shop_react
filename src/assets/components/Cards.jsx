...
export default function MainCard({
  items: { id, category, image, title, price },
}) {
  ...
  return (
    <>
      <div
        ...
        id={`item${id}`}
        className="flex flex-col gap-2 justify-between items-center w-52 hover:shadow-[0_0_0_2px_#D10024] py-2 px-4 shadow-[0_0_0_1px_#E4E7ED] h-[500px] text-center cursor-pointer duration-100  "
      >
        ...
        <div className="flex justify-between items-center gap-2">
          {cart.some((item) => item.id === id) ? (
            <div className="flex justify-between w-full gap-4">
              <button
                className=" cursor-pointer hover:bg-black bg-global-red text-white px-2 rounded-full"
                onClick={(event) => addToCart(id, price, event)}
              >
                +
              </button>
              <input
                ...
                value={cart.find((item) => item.id === id).amount || 1}
                className="border border-global-red w-16 rounded-2xl px-2 py-1"
                type="number"
              />
              <button
                className=" cursor-pointer hover:bg-black bg-global-red text-white px-2 rounded-full"
                onClick={(event) => removeFromCart(id, event, price)}
              >
                -
              </button>
            </div>
            ...
            <button
              onClick={(event) => addToCart(id, price, event)}
              className="bg-global-red text-sm animate-pulse [animation-iteration-count:1] px-2 py-1.5 w-full capitalize text-white rounded-3xl cursor-pointer hover:bg-black duration-200"
            >
              add to cart
            </button>
          )}
...
export function SingleCard({
  data: { id, category, image, title, price, rating, description },
}) {
  const { cart, addToCart, removeFromCart } = useCartStore();
  const inpRef = useRef(null);
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:justify-start sm:items-start gap-4 justify-between items-center max-w-container mx-auto w-[90%]">
        <img
          className=" object-contain rounded-2xl aspect-square h-96 shadow-[0_5px_20px_0_rgba(0,0,0,0.1)] animate-pulse [animation-iteration-count:1]"
          src={image}
          alt=""
        />
        <div className="flex flex-col gap-3 justify-between items-center sm:items-start">
          ...
          <div className="flex flex-col gap-2 max-sm:text-center" id="desc">
            <h1 className="text-3xl border-b w-[250px] mx-auto py-2.5 sm:w-full">
              Description
            </h1>
            <p>{description}</p>
          </div>
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:justify-start sm:items-start gap-4 justify-between items-center max-w-container mx-auto w-[90%] animate-pulse">
        <img
          className=" object-contain rounded-2xl bg-gray-200  aspect-square h-96 "
          alt=""
        />
        <div className="flex flex-col  gap-3 justify-between items-center sm:items-start">
          ...
          <div
            className="flex flex-col  gap-2 max-sm:text-center mt-24"
            id="desc"
          >
    ...
    <>
      {isLoading ? (
        ...
        <div className="flex gap-2.5 flex-col w-full items-center shadow-2xl overflow-hidden rounded-2xl px-2.5 py-2">
          <img
            className=" aspect-square object-contain h-96 border-b"
            src={data.image}
            alt=""
          />
          ...
          <span className="text-xl mr-2">
            price:
            <span className="text-global-red text-2xl">${data.price}</span>
          </span>
          <div className="">
            ...
            {cart.some((item) => item.id === id) ? (
              <div className="flex  w-full gap-4">
                <button
                  className=" cursor-pointer hover:bg-black bg-global-red text-white px-2 rounded-full"
                  onClick={(event) => addToCart(id, data.price, event)}
                >
                  +
                </button>
                <input
                ...
                <button
                  className=" cursor-pointer hover:bg-black bg-global-red text-white px-2 rounded-full"
                  onClick={(event) => removeFromCart(id, event, data.price)}
                >
                  -
                </button>
              </div>
  ...
  return (
    <>
      <div
        ...
        className="flex py-1 px-2 cursor-pointer hover:text-global-red hover:scale-[1.1] bg-white hover:rounded-2xl border-b border-black gap-2.5 h-14 items-center w-full"
      >
        <img className="h-full aspect-square " src={image} alt="" />
        <h1 className="text-center text-sm w-[200px] overflow-hidden">
          {title}
        </h1>
