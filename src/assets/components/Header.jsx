...
export default function Header() {
  ...
  const categoriesFetch = useCallback(() => {
    allProductsFetch("https://fakestoreapi.com/products/categories").then(
    ...
    <>
      <header className="text-white">
        <div id="tHeader" className="w-full h-max bg-[#1E1F29]">
          <div
            id="tHeader-container"
            className="max-w-container w-[90%] flex justify-between mx-auto flex-wrap content-between"
          >
        ...
        <div
          id="Bheader"
          className="w-full h-max bg-[#15161D] border-b-4 border-global-red"
        >
          <div
            id="bHeader-container"
            className="w-[90%] max-w-container mx-auto flex flex-col gap-4 flex-wrap md:flex-row justify-between items-center py-3"
          >
            <img
            ...
            <div className="flex justify-center rounded-4xl overflow-x-hidden-hidden relative ">
              ...
              <input
                className="bg-white md:w-[350px] w-[60%] text-black border-l px-2"
                type="text"
                name=""
                id=""
                ...
                onClick={() =>
                  setFiltered(
                    data.filter((item) =>
                      item.title
                        .toLowerCase()
                        .includes(inp.current.value.toLowerCase())
                    )
                  )
                }
                onChange={() =>
                  setFiltered(
                    data.filter((item) =>
                      item.title
                        .toLowerCase()
                        .trim()
                        .includes(inp.current.value.toLowerCase().trim())
                    )
                  )
                }
                ref={inp}
              ...
              <Button
                ...
                className="!bg-global-red !rounded-[0px]"
                variant="contained"
              >
                search
              </Button>
              <div
                ref={filterMenu}
                className={`md:w-[77%] w-full z-50 flex pt-2 rounded-2xl border-x border-global-red  text-black flex-col gap-2 items-center  absolute h-max left-0 md:left-[131px] top-[44px]  bg-white ${
                  filtered.length === 0 || inp.current.value === ""
                    ? "hidden"
                    : "block"
                } `}
              >
            ...
            <div
              ...
              className="flex  group cursor-pointer justify-between flex-col items-center relative"
            >
              <div className="rounded-full w-5 text-sm text-black text-center bg-global-red right-0 top-[-13px] absolute">
                {cart
              ...
              >
                <path
                  className="group-hover:stroke-global-red duration-300"
              ...
              <span className="group-hover:text-global-red duration-300">
                your cart
              </span>
            </div>
          </div>
        ...
        <div className="w-[90%] max-w-container mx-auto flex gap-2 flex-wrap py-2 sm:justify-start justify-center sm:text-xl text-sm ">
          {categories.map((item) => (
            <h1
              ...
              className="px-2 border-l hover:text-global-red cursor-pointer"
            >
              {item}
            </h1>
          ))}
